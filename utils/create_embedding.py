...
def create_movie_embedding(movies_file_path: str, 
    ...
    unique_genome_tags = set()
    for tags in movie_genome_tags:
        unique_genome_tags.update([tag for tag in tags])
        
    model = SentenceTransformer(model_name)
    
    print(f"Creating movie embeddings for {len(unique_genome_tags)} tags...")
    tag_to_embeddings = {}
    for tag in tqdm(unique_genome_tags):
    ...
    for movie_id, tags in tqdm(zip(movies['movieId'], movie_genome_tags)):
        if len(tags) > 0:
            embeddings = np.mean([tag_to_embeddings[tag] for tag in tags], axis=0)
            movie_embeddings[movie_id] = embeddings
        else:
